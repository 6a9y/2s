				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Premium Scripts")
							if not fold then
								makefolder("./Premium Scripts")
							end
							fold = isfolder("./Premium Scripts/Themes")
							if not fold then
								makefolder("./Premium Scripts/Themes")
							end
							fold = isfolder("./Premium Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Premium Scripts" then
		library.WorkspaceName = convertfilename(windowName, "remium Scripts")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Premium Scripts") then
						makefolder("./Premium Scripts")
					end
					local common_string = "./Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Premium Scripts Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
